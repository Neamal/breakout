...
public class Breakout extends GDV5{
	...
	public void update() {
		p1.controlPaddle();
		m1.intersectBrick(bricks);
		if (!freeze) m1.fire(m1, p1);
		if(m1.getVis() == false) {
			m1.setLocation((int)p1.getCenterX(), 740);
		}
		s.P1Score = P1Score;
		for (int i = 0; i < bricks.length; i++) {
			if (bricks[i].getVis() == true && m1.intersects(bricks[i])) {
				P1Score ++;
				s1.play(0);
			}
		}
	...
	public void draw(Graphics2D win) {
		...
		if (!start) {
			...
			win.drawString("Space Invaders", Breakout.getMaxWindowX()/2 - 170, 100);
			win.setFont(new Font("Arial", Font.BOLD, 20));
			win.drawString("By: Junhyung Yoon, Eric Nam, Raffi Dulgerian", Breakout.getMaxWindowX()/2 - 200, 150);
			...
			win.drawString("Press 'v' to to shoot your bullets!", 140, 420);
			lives = 5;
			P1Score = 0;
			b1.setLocation(GDV5.getMaxWindowX()/2, GDV5.getMaxWindowY() - 100);
